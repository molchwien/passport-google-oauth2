...
var OAuth2Strategy = require('passport-oauth2')
  , util = require('util')
  , uri = require('url')
  , Profile = require('./profile/openid')
...
function Strategy(options, verify) {
  ...
  options.tokenURL = options.tokenURL || 'https://auth.molch.at/oauth2/jwks/index.pl';
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    if (err) {
      ...
      if (json && json.error && json.error.message) {
        return done(new MolchAPIError(json.error.message, json.error.code));
